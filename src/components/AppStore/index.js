...
class AppStore extends Component {
  state = {tabSelected: 'SOCIAL', userSearch: ''}
  ...
  render() {
    const {tabSelected, userSearch} = this.state
    const filteredList = appsList.filter(
      eachItem => eachItem.category === tabSelected,
    )
    const resultList = filteredList.filter(eachItem =>
      eachItem.appName.toLowerCase().includes(userSearch.toLowerCase()),
    )
        ...
        <ul className="tab-container">
          {tabsList.map(eachItem => (
            <TabItem
              ...
              isSelected={eachItem.tabId === tabSelected}
              key={eachItem.tabId}
            />
          ))}
        </ul>
